...
async function parse_header(uuid_str, client) {
    ...
    try {
        ...
        log('debug', 'VLS header parsed successfully');
        return vless
    } catch (err) {
        log('error', `VLS header parse error: ${err.message}`);
        throw new Error(`read vls header error: ${err.message}`)
    } finally {
        reader.releaseLock()
    }
}
...
class Session {
    ...
    async initializeVLESS(firstPacket) {
        ...
        try {
            log('debug', 'Initializing VLS connection from first packet');
            ...
            log('info', `VLS header parsed: ${this.vlessHeader.hostname}:${this.vlessHeader.port}`);
            ...
            log('error', `Failed to initialize VLs: ${err.message}`);
            return false;
        }
    }
    ...
    _startDownstreamResponse() {
        ...
        try {
            ...
            if (!this.headerSent) {
                log('debug', `Sending response header (${protocol}): ${this.responseHeader.length} bytes`);
